...
test('should be able to fetch partition info info - GET /db/_partition/partition - db.partitionInfo', async () => {
  ...
  const p = await couch.db._partition.partition()
  assert.deepEqual(p, response)
  mockAgent.assertNoPendingInterceptors()
})

test('should handle missing database - PUT /db - couch.db.create', async () => {
  // mocks
  const response = {
    error: 'not_found',
  ...
  // test GET /db/_partition/partition
  await assert.rejects(couch.db._partition.partition(), { message: response.reason })
  mockAgent.assertNoPendingInterceptors()
})
