...
test('should be able to fetch a list of documents - POST /db/_all_docs - db.fetch', async () => {
  ...
  const p = await couch.db._all_docs({ method: 'post', qs: { include_docs: true }, body: { keys }})
...
test('should be able to fetch a list of documents with opts - POST /db/_all_docs - db.fetch', async () => {
  ...
  // test POST /db/_all_docs
  const p = await couch.db._all_docs({ method: 'post', qs: { include_docs: true, descending: true }, body: { keys }})
  assert.deepEqual(p, response)
  mockAgent.assertNoPendingInterceptors()
})
...
test('should be able to handle 404 - POST /db/_all_docs - db.fetch', async () => {
  ...
  // test POST /db/_all_docs
  await assert.rejects(couch.db._all_docs({ method: 'post', qs: { include_docs: true }, body: { keys }}), { message: 'missing' })
  mockAgent.assertNoPendingInterceptors()
})
