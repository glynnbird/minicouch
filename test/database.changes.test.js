...
test('should be able to fetch the changes - GET /db/_changes - couch.db.changes', async () => {
  // mocks
  mockPool
  ...
  const p = await couch.db._changes()
  assert.deepEqual(p, response)
  mockAgent.assertNoPendingInterceptors()
})

test('should be able to fetch the changes with opts - GET /db/_changes - couch.db.changes', async () => {
  // mocks
  ...
  // test GET /db/_changes
  const p = await couch.db._changes({ qs: opts })
  assert.deepEqual(p, response)
  mockAgent.assertNoPendingInterceptors()
})

test('should be able to handle a missing database - GET /db/_changes - couch.db.changes', async () => {
  // mocks
  mockPool
    .intercept({ path: '/db/_changes' })
  ...
  // test GET /db/_changes
  await assert.rejects(couch.db._changes(), { message: 'Database does not exist.' })
  mockAgent.assertNoPendingInterceptors()
})
