...
test('should be able to fetch a document with attachments - multipart GET /db - db.multipart.get', async () => {
  ...
  const p = await couch.db.docid({ headers: { accept: 'multipart/related'}, qs: { attachments: true } })
  assert.deepEqual(p, Buffer.from(multipartResponse))
...
test('should be able to fetch a document with attachments with opts - multipart GET /db - db.multipart.get', async () => {
  ...
  const p = await couch.db.docid({ headers: { accept: 'multipart/related' }, qs: { conflicts: true, attachments: true }})
  assert.deepEqual(p, Buffer.from(multipartResponse))
  mockAgent.assertNoPendingInterceptors()
})
...
test('should be able to handle 404 - db.multipart.get', async () => {
  ...
  // test GET /db
  await assert.rejects(couch.db.docid({ headers: { accept: 'multipart/related' }, qs: {  attachments: true }}), { message: 'missing' })
  mockAgent.assertNoPendingInterceptors()
})
