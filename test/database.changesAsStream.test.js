...
test('should get a streamed list of changes - GET /_changes - couch.db.changesAsStream', async () => {
  // mocks
  mockPool
  ...
  const s = await couch.db._changes({ stream: true })
  let buffer = ''
  s.on('data', (chunk) => {
...
test('should get a streamed list of changes with opts - GET /_changes - couch.db.changesAsStream', async () => {
  // mocks
  ...
  // test GET /db/_changes
  const s = await couch.db._changes({ qs: opts, stream: true })
  let buffer = ''
  s.on('data', (chunk) => {
    buffer += chunk.toString()
  })
