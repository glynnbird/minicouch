...
test('should be able to fetch db updates - GET /_db_updates - couch.updates', async () => {
  // mocks
  mockPool
  ...
  const p = await couch._db_updates()
  assert.deepEqual(p, response)
  mockAgent.assertNoPendingInterceptors()
})

test('should be able to fetch db updates with options - GET /_db_updates - couch.updates', async () => {
  // mocks
  mockPool
  ...
  // test GET /_db_updates
  const p = await couch._db_updates({ qs: { timeout: 10000 }})
  assert.deepEqual(p, response)
  mockAgent.assertNoPendingInterceptors()
})

test('should handle 404 - GET /_db_updates - couch.updates', async () => {
  // mocks
  mockPool
    .intercept({ path: '/_db_updates' })
  ...
  // test GET /_db_updates
  await assert.rejects(couch._db_updates(), { message: 'Database does not exist.' })
  mockAgent.assertNoPendingInterceptors()
})
