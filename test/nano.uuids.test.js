...
test('should be able to fetch uuids - GET /_uuids - couch.uuids', async () => {
  // mocks
  const response = {
    uuids: [
  ...
  // test GET /_uuids
  const p = await couch._uuids({ qs: { count: 1 }})
  assert.deepEqual(p, response)
  mockAgent.assertNoPendingInterceptors()
})

test('should be able to fetch more uuids - GET /_uuids?count=3 - couch.uuids', async () => {
  // mocks
  const response = {
    uuids: [
  ...
  // test GET /_uuids
  const p = await couch._uuids({ qs: { count: 3 }})
  assert.deepEqual(p, response)
  mockAgent.assertNoPendingInterceptors()
})
