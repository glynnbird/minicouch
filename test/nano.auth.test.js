...
test('should be able to handle cookie refresh - POST /_session - nano.auth', async () => {
  ...
  // test POST /_session
  const p1 = await couch._session({ method: 'post', body: 'name=u&password=p', headers: { 'content-type': 'application/x-www-form-urlencoded; charset=utf-8' }})
  assert.deepEqual(p1, response)
  const p2 = await couch._all_dbs()
  assert.deepEqual(p2, ['a'])
  const p3 = await couch._all_dbs()
