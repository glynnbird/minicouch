...
test('should list documents form a partition - GET /db/_partition/_all_docs - db.partitionedList', async () => {
  // mocks
  mockPool
  ...
  const p = await couch.db._partition.partition._all_docs()
  assert.deepEqual(p, response)
  mockAgent.assertNoPendingInterceptors()
})

test('should list documents form a partition with opts - GET /db/_partition/_all_docs - db.partitionedList', async () => {
  // mocks
  const optsResponse = {
    total_rows: 1215,
    offset: 0,
  ...
  // test GET /db/_partition/_all_docs
  const p = await couch.db._partition.partition._all_docs({ qs: { limit: 1, include_docs: true }})
...
test('should escape unusual characters - GET /db/_partition/a+b/_all_docs - db.partitionedList', async () => {
  ...
  // test GET /db/_partition/_all_docs
  const p = await couch.db._partition['a+b']._all_docs()
  assert.deepEqual(p, response)
  mockAgent.assertNoPendingInterceptors()
})
...
test('should handle missing database - GET /db/_partition/_all_docs - db.partitionedList', async () => {
  ...
  // test GET /db/_partition/_all_docs
  await assert.rejects(couch.db._partition.partition._all_docs, { message: errResponse.reason })
  mockAgent.assertNoPendingInterceptors()
})
